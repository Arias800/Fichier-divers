...
def showMovies(sSearch = ''):
    ...
    if Nextpagesearch:
        sSearch = sUrl.replace('https','http')
    ...
    if sSearch:
        ...
        oRequestHandler = cRequestHandler(URL_SEARCH[0].replace('https','http'))
        oRequestHandler.setRequestType(cRequestHandler.REQUEST_TYPE_POST)
        oRequestHandler.addParametersLine(data)
        oRequestHandler.addParameters('User-Agent', UA)
        oRequestHandler.addParameters('Accept','text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8')
        oRequestHandler.addParameters('Accept-Encoding', 'gzip, deflate')
