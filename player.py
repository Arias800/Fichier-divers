    ...
    def run(self, oGuiElement, sTitle, sUrl):
        ...
        if sUrl.endswith('.mpd') or sUrl.split('?')[0][-4:] in '.mpd':
            if isKrypton() == True:
                ...
                if "viki" in sUrl:
                    headers = 'User-Agent={}&Host={}&Referer={}&Origin={}'.format(UA, "manifest-viki.viki.io","https://www.viki.com","https://www.viki.com")
                    license_server_url = "https://manifest-viki.viki.io/v1/license?dt=dt3&video_id=1128037v&app=100000a&app_ver=4.0.84"
                    item.setProperty('inputstream.adaptive.license_key', '{}|{}|{}|{}'.format(license_server_url, headers, "R{SSM}", ""))
